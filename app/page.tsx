...
function DiagramaFlujoMobile() {
  /* ── Pipeline vertical: grupos compactos con tuberías S entre etapas ── */
  ...
  /* Grupos más compactos, cajas más juntas */
  const fuentes: Caja[] = [
    { id: "ERP", x: 80, y: 38, w: 110, h: 34, color: "#64748B" },
    { id: "CRM", x: 280, y: 38, w: 110, h: 34, color: "#64748B" },
    { id: "Planillas", x: 80, y: 82, w: 110, h: 34, color: "#64748B" },
    { id: "Soporte", x: 280, y: 82, w: 110, h: 34, color: "#64748B" },
  ];

  const proceso: Caja[] = [
    { id: "Ingesta", x: 80, y: 168, w: 110, h: 34, color: "#0EA5E9" },
    { id: "Gobierno", x: 280, y: 168, w: 110, h: 34, color: "#0EA5E9" },
  ];

  const almacen: Caja[] = [
    { id: "Lakehouse", x: 180, y: 242, w: 150, h: 40, color: "#2563EB" },
  ];

  const modelos: Caja[] = [
    { id: "ML", x: 80, y: 322, w: 110, h: 34, color: "#0EA5E9" },
    { id: "DataSci", x: 280, y: 322, w: 110, h: 34, color: "#0EA5E9" },
  ];

  const salidas: Caja[] = [
    { id: "BI", x: 80, y: 408, w: 110, h: 34, color: "#2563EB" },
    { id: "NLP", x: 280, y: 408, w: 110, h: 34, color: "#2563EB" },
  ];

  const grupos = [
    { key: "fuentes", label: "FUENTES", y0: 10, y1: 120, color: "#64748B", cajas: fuentes },
    { key: "proceso", label: "PROCESO", y0: 120, y1: 210, color: "#0EA5E9", cajas: proceso },
    { key: "almacen", label: "ALMACÉN", y0: 210, y1: 290, color: "#2563EB", cajas: almacen },
    { key: "modelos", label: "ML / IA", y0: 290, y1: 370, color: "#0EA5E9", cajas: modelos },
    { key: "salidas", label: "REPORTES", y0: 370, y1: 455, color: "#2563EB", cajas: salidas },
  ];

  // ── Tuberías S entre grupos (conectan punto medio inferior → punto medio superior) ──
  const conexiones = [
    { from: { x: 180, y: 120 }, to: { x: 180, y: 210 }, color: "#0EA5E9" },   // fuentes → proceso
    { from: { x: 180, y: 210 }, to: { x: 180, y: 290 }, color: "#2563EB" },   // proceso → almacén
    { from: { x: 180, y: 290 }, to: { x: 180, y: 370 }, color: "#0EA5E9" },   // almacén → modelos
    { from: { x: 180, y: 370 }, to: { x: 180, y: 455 }, color: "#2563EB" },   // modelos → reportes
  ];

  const tuberias = conexiones.map((c, i) => {
    const x1 = c.from.x;
    const y1 = c.from.y;
    const x2 = c.to.x;
    const y2 = c.to.y;
    ...
    // Curva S pronunciada
    const cp1x = x1 + (i % 2 === 0 ? 60 : -60);
    const cp1y = y1 + dy * 0.35;
    const cp2x = x2 + (i % 2 === 0 ? -60 : 60);
    const cp2y = y2 - dy * 0.35;

    const d = `M ${x1} ${y1} C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${x2} ${y2}`;
    return { d, delay: i * 0.5, color: c.color };
  });

  // Partículas de datos
  const particulas = tuberias.flatMap((t, ti) => [
    { key: `p-${ti}-1`, path: t.d, delay: t.delay + 0.15, color: t.color, r: 3.5 },
    { key: `p-${ti}-2`, path: t.d, delay: t.delay + 0.45, color: t.color, r: 2.5 },
    { key: `p-${ti}-3`, path: t.d, delay: t.delay + 0.75, color: "#FFFFFF", r: 2 },
  ]);
  ...
  return (
    <div className="md:hidden w-full">
      ...
      >
        <defs>
          <filter id="pipeGlow2" x="-50%" y="-50%" width="200%" height="200%">
          ...
          <filter id="cajaShadow2" x="-10%" y="-10%" width="120%" height="130%">
        ...
        {/* ── Cajas dentro de cada grupo ── */}
        {grupos.flatMap((g) =>
          g.cajas.map((c) => (
            <g key={c.id} filter="url(#cajaShadow2)">
              {/* Caja */}
              <rect
                x={c.x - c.w / 2}
                y={c.y - c.h / 2}
